...
function multiplyEntries(res) {
    ...
    // If the show passed into the next year AND the episode count is larger than a full year move duplicate it
    if (startYear && endYear && startYear !== endYear && res["Episodes"] > MAX_EPISODES_PER_YEAR) {
        let output = [];
