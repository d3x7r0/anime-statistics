...
let Utils = require("sukurapa").Utils,
...
function ANNReader() {
    if (!(this instanceof ANNReader)) {
        return new ANNReader();
    }
}

ANNReader.prototype.prepare = function prepare() {
    this._page = START_PAGE;
    this._continue = true;
};

ANNReader.prototype.next = function next() {
    return getPage(this._page).then(data => {
...
ANNReader.prototype.hasNext = function hasNext() {
    return this._continue;
};

ANNReader.prototype.finish = function finish() {
...
function fetchPage(page) {
    ...
    return Utils.queue(fetchXML, url).then(
        data => ensureArray(data && data["report"] && data["report"]["item"])
            .map(item => item["id"])
    );
}

function fallbackEntriesFetch(err, ids) {
...
function fetchEntries(ids) {
    let params = ensureArray(ids)
        .map(id => `title=${id}`)
        .join("&");

    return Utils.queue(fetchXML, SINGLE_URL + params)
        .then(data => ensureArray(data && data["ann"] && data["ann"]["anime"]));
}
