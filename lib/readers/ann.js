...
function fetchEntries(ids) {
    var params = [].concat(ids || [])
        .map(id => `title=${id}`)
        .join("&");

    return fetchXML(SINGLE_URL + params);
}
...
function processEntry(data) {
    ...
    if (res["isAdaptation"]) {
        ...
        output = Bluebird.delay(Utils.DEFAULT_DELAY, relations)
            .then(fetchEntries)
            .then(data => {
